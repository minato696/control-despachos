...
import { faCity, faPlus, faSearch, faClipboardList } from '@fortawesome/free-solid-svg-icons'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredCities.map((city) => {
          ...
          return (
            <div 
              key={city}
              className="border border-gray-light rounded shadow p-6 hover:shadow-md hover:translate-y-[-3px] transition-all cursor-pointer h-full flex flex-col"
            >
              <div className="flex justify-between items-center pb-4 border-b border-gray-light mb-4">
                <h3 className="text-lg font-semibold text-primary-darker">
                  {cityNameMap[city] || city}
                </h3>
                <span className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold bg-primary-light text-primary">
              ...
              <ul className="flex-1">
                {cityReporters.map(reporter => (
                  <li key={reporter.id} className="py-3 flex items-center gap-3 border-b border-gray-light last:border-b-0">
                    <FontAwesomeIcon icon={faSearch} className="text-gray" />
                    {reporter.nombre}
                  </li>
                ))}
              ...
              <div className="mt-4">
                <button 
                  ...
                  className="w-full flex items-center justify-center gap-2 px-3 py-1.5 text-sm bg-white text-primary border border-primary-border rounded shadow-sm hover:bg-primary-light transition-colors"
                >
